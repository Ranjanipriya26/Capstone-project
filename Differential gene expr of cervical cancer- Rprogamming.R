...
path <- paste(urld, "acc=GSE120691", "file=GSE120691_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
# sample selection
gsms <- "000000000000000000000000000000001111111111111111111"
sml <- strsplit(gsms, split="")[[1]]

# group membership for samples
gs <- factor(sml)
groups <- make.names(c("Cervical cancer","Healthy control"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE120691 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE120691 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE120691", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
